...
- `stdin_delimiter` — `None` (default): raw chunks as they arrive; or a delimiter
  ...
  when `stdin_want_bytes=True`; a mismatch (or an empty delimiter) raises `ValueError` at
  config time;
- `stdin_want_bytes` — `False` (default): `data` is `str`, decoded with an **incremental
