...
def _enter_raw():
    ...
    The alt-screen enter is followed by an explicit clear (``\\033[2J``):
    ``?1049h`` only clears the alt buffer when it actually *switches* into
    it, which it does NOT when we are handed an already-active alt screen by
    a parent recipe that kept it (``run_external(keep_screen=True)``). The
    differential renderer dropped the historical blanket clear and relies on
    a blank buffer at entry, so without this a nested child would paint over
...
# CSI modifier parameter -> key-name prefix (xterm encoding: the second
# numeric parameter is 1 + a bitmask of Shift=1, Alt=2, Ctrl=4). Only
# these five combinations are decoded — anything else (e.g. alt-shift)
# falls back to the bare key name.
_CSI_MOD_PREFIX = {
    2: 'shift-',
    3: 'alt-',
    5: 'ctrl-',
...
# Tilde-sequence keycode -> key name (ESC [ <num> ~).
_CSI_TILDE_KEYS = {
    '1': 'home', '7': 'home',
    '4': 'end', '8': 'end',
    '2': 'insert', '3': 'delete',
...
def _read_csi(fd, _read1, _peek):
    ...
    if buf.endswith('~'):
        ...
        mod_prefix = ''
        if ';' in raw:
            num, _, mod_s = raw.partition(';')
            try:
                mod_prefix = _CSI_MOD_PREFIX.get(int(mod_s), '')
            except ValueError:
                pass
        else:
            num = raw
    ...
    # Shift/Ctrl/Alt modified arrows, Home/End, and F1-F4:
    # ESC [ 1 ; <mod> <A-D|H|F|P-S> — the modifier sits in the second
    # parameter, the final letter selects the key. xterm-family
    # terminals send *modified* F1-F4 in this form (bare F1-F4 arrive
    # as SS3 ESC O P..S, handled in ``read_key``).
    if len(buf) >= 3 and buf[-1] in 'ABCDHFPQRS' and ';' in buf:
        key_name = {
        ...
        if len(parts) == 2:
            try:
                ...
                return key_name
            return _CSI_MOD_PREFIX.get(mod, '') + key_name
        return key_name

    # SGR mouse: ESC [ < Cb ; Cx ; Cy M/m
    #
    # ``Cb`` is a bitfield (xterm SGR encoding):
    #   bits 0-1 — button: 0=left, 1=middle, 2=right
    #   bit 2 (+4)  — Shift
    #   bit 3 (+8)  — Alt (Meta)
    #   bit 4 (+16) — Ctrl
    #   bit 5 (+32) — motion/drag (only under ?1002/?1003, which we don't
    #                 enable — so it never arrives and is not decoded here)
    #   bit 6 (+64) — wheel / extended button (64 up, 65 down, 66 left,
    #                 67 right)
    # The final byte is 'M' for press and 'm' for release.
    if buf.startswith('<') and buf[-1] in ('M', 'm'):
        ...
        if len(parts) == 3:
            try:
                ...
                return '_mouse'
            # Wheel / extended-button family (bit 6). The low two bits
            # select the direction. Wheel notches carry no press/release
            # semantics, so the modifier bits and final byte are ignored
            # here — the bare cb 64/65 case stays byte-for-byte as before.
            if cb & 64:
                return ('scroll-up', 'scroll-down',
                        'scroll-left', 'scroll-right')[cb & 3] + \
                    ':{}:{}'.format(cy, cx)
            # Translate the SGR modifier bits (Shift=4, Alt=8, Ctrl=16)
            # into the xterm *keyboard* modifier parameter (1 + Shift=1,
            # Alt=2, Ctrl=4) so we reuse _CSI_MOD_PREFIX verbatim — mouse
            # modifier names then match key modifier names exactly, and
            # unsupported combos (e.g. shift-alt) fall back to no prefix.
            kbd_mask = (
                (1 if cb & 4 else 0)    # Shift
                | (2 if cb & 8 else 0)  # Alt
                | (4 if cb & 16 else 0)  # Ctrl
            )
            mod = _CSI_MOD_PREFIX.get(kbd_mask + 1, '') if kbd_mask else ''
            if buf[-1] == 'm':  # button release (any button)
                return '{}mouse-release:{}:{}'.format(mod, cy, cx)
            button = cb & 3
            if button == 0:
                return '{}mouse-click:{}:{}'.format(mod, cy, cx)
            if button == 1:
                return '{}middle-click:{}:{}'.format(mod, cy, cx)
            if button == 2:
                return '{}right-click:{}:{}'.format(mod, cy, cx)
            return '_mouse'  # button 3 (legacy release sentinel) — drop
        return '_mouse'
