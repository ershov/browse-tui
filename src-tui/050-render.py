...
def render_info_bar(row, cols, label, *, info=False, browser=None,
    ...
    When ``info=True`` the bar additionally shows:
      ...
      * an info-bar notice (``browser._notice``) in place of the hint —
        red+bold for an error, a reverse-video yellow band for a flash;
    ...
    if search is not None and pos < cols:
        ...
        # budget. Red + bold for errors (loud); a reverse-video yellow
        # band for flashes — an eye-catching acknowledgement the user
        # can't miss (mutations rely on it as their only feedback).
        ...
        if avail > 10:
            ...
            if notice.kind == 'error':
                ...
                set_style(fg=11, reverse=True, bold=True)
            write(text)
            pos += len(text)
        reset_style()
        set_style(fg=8)
