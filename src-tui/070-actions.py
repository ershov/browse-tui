...
def _run_external_on_preview(ctx, *, env_var, default):
    ...
    v/e mean "give me the WHOLE thing", so this always resolves the
    *complete* preview — including for streaming (generator) previews,
    whose cached ``item.preview`` is only a partial prefix while the
    worker is paused at the buffer cap (``_stream_preview_from_generator``).

    Resolution order for the preview text:

      1. ``item.preview`` — used as-is when present (including the empty
         string, legitimate empty content) AND the cache is complete,
         i.e. ``browser.preview_cache_is_partial(item.id)`` is ``False``.
         Covers string previews, exhausted generators and push-mode
         content.
      2. Otherwise (cache absent, or a paused partial stream) a
         synchronous fetch via ``browser.get_preview(item.id)``. A
         generator result is drained to its end — that's the fix for
         the truncated-pager bug. A drained generator is NOT written
         back to ``item.preview``: a live worker may still hold a
         paused generator for this id whose scroll-resume appends from
         where it left off, so overwriting its cache would desync it.
         A non-generator string IS cached on a true cache miss (the
         old pane-toggle optimisation, unchanged).
      3. If nothing is available — ``browser.get_preview`` is ``None``
         or the fetcher returned ``None``/raised — surface a message
         and skip the external command (falling back to a partial cache
         when one exists rather than showing nothing).
    ...
    if get_preview is None:
        ...
        return
    cached = getattr(item, 'preview', None)
    if cached is not None and not ctx._browser.preview_cache_is_partial(item.id):
        # Complete cache — use it as-is (string preview, exhausted
        # generator, or push-mode content).
        text = cached
    else:
        # Cache miss OR a paused partial stream — fetch synchronously and
        # fully materialise. The preview pane may be hidden, the async
        # worker may not have run yet, or it may have parked a generator
        # at the buffer cap; either way the user pressed v/e meaning "I
        # want the whole thing now", so blocking briefly on the recipe's
        # get_preview is acceptable.
        try:
            result = get_preview(item.id)
        except Exception as e:
            ctx.error(f'preview: {type(e).__name__}: {e}')
            return
        if inspect.isgenerator(result):
            # Streaming pull source — drain to the end for the complete
            # text. Coerce chunks like the streaming worker does. Do NOT
            # cache the drained text: a live worker may still hold a
            # paused generator for this id and overwriting its partial
            # cache would desync the scroll-resume path.
            try:
                text = ''.join(
                    chunk if isinstance(chunk, str)
                    else ('' if chunk is None else str(chunk))
                    for chunk in result
                )
            except Exception as e:
                ctx.error(f'preview: {type(e).__name__}: {e}')
            ...
            text = result
            # Cache the full string on a true cache miss so a later
            # preview-pane toggle skips the refetch (unchanged behaviour).
            if cached is None:
                item.preview = text
        elif cached is not None:
            # Re-fetch yielded nothing usable — fall back to the partial
...
def dispatch_key(browser, ctx: 'Context', key: str) -> bool:
    ...
    """
    # Mouse events (clicks + wheel) are dispatched first regardless of
    # mode. ``_dispatch_mouse`` parses the row/col, looks up the pane
    # via ``layout_panes``, and applies the per-pane behaviour. Search
    # and filter edit modes keep their prompt open — the mouse just
    ...
    if browser._mode is Mode.SEARCH_EDIT:
        ...
        # composing a query. Mouse events are dispatched at the top of
        # this function regardless of mode, so they never reach here.
