...
class _Button:
    """One parsed button: its on-screen display, hotkey, and return label.

    The constructor resolves the standard ``&`` hotkey convention once so
    drawing and key handling read plain fields:

      * ``display`` — what's shown inside the ``[ … ]`` cell (markers
        resolved: ``&X`` → ``X``, ``&&`` → a literal ``&``).
      * ``label`` — the value :meth:`ChoiceContent.handle_key` returns for
        this button. Same as ``display`` — the markers are resolved away.
...
class ChoiceContent:
    ...
    ``buttons`` is a sequence of label strings using the standard ``&``
    hotkey convention (``'&Yes'`` → ``Yes`` with ``Y`` the hotkey; ``&&`` a
    literal ``&``; no ``&`` → no hotkey). An empty ``buttons`` raises
    ``ValueError`` — a programming error, not a user condition. With a single
    button (the alert case) ``space`` also activates it.

    :meth:`handle_key` returns the chosen button's label with the markers
    resolved (``'&Yes'`` → ``'Yes'``); the engine's cancel path returns
    ``None``. Implements the content protocol consumed by :func:`run_modal`
    (``title`` / ``measure`` / ``draw_row`` / ``handle_key``).
    """
    ...
    def handle_key(self, key):
        ...
        ``right``). ``enter`` closes with the focused button's resolved label.
        A hotkey letter (case-insensitive) closes with THAT button's label
        immediately, regardless of focus. With a single button, ``space`` also
        activates it. Every other key is ignored.
        """
        n = len(self._buttons)
        if key in ('right', 'tab'):
        ...
        if key == 'left':
            ...
            return (False, None)
        if key == 'enter':
            return (True, self._buttons[self.focus].label)
        if key == 'space' and n == 1:
            return (True, self._buttons[0].label)
