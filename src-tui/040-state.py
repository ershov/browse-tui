    ...
    def run(self) -> int:
        ...
        try:
            while not self._quit_requested:
                ...
                # order (spec "Ordering within a drain"): geometry
                # (on_resize) and input state (on_search_change →
                # on_filter_change) first since they may reshape the visible
                # tree, then structure (on_collapse → on_expand), then
                # children-arrival (on_children_loaded) before cursor
                # settling, so a cursor-change handler sees the
                # post-expansion tree with freshly-delivered children
                # accounted for. (on_resize consumes the ``_resize_pending``
