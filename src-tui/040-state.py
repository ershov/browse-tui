    ...
    def append_preview(self, id_, chunk) -> None:
        ...
        def _apply():
            ...
            if item is not None:
                ...
                # Wrap cache (#423) — try to extend the existing wrap in
                # place using the recorded tail offsets so streaming
                # appends don't pay an O(N) re-wrap on every chunk. Falls
                # back to dropping the cache (next paint regens) when the
                # cache is absent, when chunk is empty, or when the
                # cached geometry / ANSI policy does not match the
                # current Browser state (defensive — eager invalidation
                # should normally prevent the mismatch).
                _extend_or_drop_preview_render(item, chunk, self.preview_ansi)
