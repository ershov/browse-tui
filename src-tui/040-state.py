...
class Browser:
    ...
    def __init__(self, config: Optional['BrowserConfig'] = None) -> None:
        ...
        # does NOT, since it drops the cache). ``on_quit`` fires once
        # during shutdown after the screen is restored.
    ...
    def run(self) -> int:
        ...
        try:
            while not self._quit_requested:
                ...
                # (on_collapse → on_expand) then children-arrival
                # (on_children_loaded) before cursor settling, so a
                # cursor-change handler sees the post-expansion tree with
                # freshly-delivered children accounted for. (The remaining
                # phases — scope, search/filter, resize — slot in here in
                # the same order as they land.)
