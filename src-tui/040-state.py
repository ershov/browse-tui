...
def _recompute_filter_hidden(state, filters, *, show_ids='auto') -> None:
    ...
    def visit(item):
        """Bottom-up DFS over the visible subtree.

        Returns True iff ``item`` should be visible
        (``_filter_hidden=False``).
        """
        # Rule 2: recipe-hidden rows aren't part of the visible tree.
        # No flag write; the renderer would skip them regardless and
        # they can't scaffold an ancestor.
        if getattr(item, 'hidden', False):
            return False

        # Descend only into expanded subtrees (Rule 1). Collapsed
