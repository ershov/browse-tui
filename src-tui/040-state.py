...
class Browser:
    ...
    def _update_preview_for_cursor(self) -> None:
        ...
        if restore is not None:
            # Drop the refresh scroll-restore once the cursor's sticky
            # anchor no longer targets the row the refresh started on — a
            # genuine re-target (user navigation, a home/end pin, or a
            # programmatic ``cursor_to``). The reload bounce keeps the
            # anchor's primary id pointing at ``restore[0]`` (that is what
            # brings the cursor back), so the transient clamp to the scope
            # row does NOT drop it. ``_cursor_anchor`` is the source of
            # truth for the sticky id; a sentinel pin or an empty anchor is
            # never equal to a concrete restore id, so both drop the slot.
            anchor = self._cursor_anchor
            if (anchor[0] if anchor else None) != restore[0]:
                self._preview_scroll_restore = None
                restore = None
        ...
        if restore is not None and new_id == restore[0]:
            # Reload bounce resolved back onto the row the refresh started
            # on (``cache_invalidate_*`` collapsed the visible tree,
            # clamping the cursor to the scope row and back). Restore its
            # scroll — the transient clamp may have reset it — and consume
            # the slot. ``_help_mode`` and the tail pin are left intact.
            self._preview_scroll = restore[1]
            self._preview_scroll_restore = None
        elif moved:
            # Fresh view on a genuinely different item: scroll to the top
            # and dismiss the help overlay so the user sees the new item's
            # preview, not stale state. ``_preview_at_tail`` is
            # intentionally NOT cleared here — the tail pin is a sticky
            # user intent that carries across cursor-item changes; the
            # renderer's pin override re-derives ``_preview_scroll =
            # max_scroll`` each pass, so this reset matters only when the
            # pin is disengaged. Cleared only by explicit upward motion.
            self._preview_scroll = 0
            if self._help_mode:
                self._help_mode = False
        if new_id is not None:
