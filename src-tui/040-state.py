    ...
    def run(self) -> int:
        ...
        try:
            while not self._quit_requested:
                ...
                # last-fired one — so a layout change this iteration paints
                # below and fires on the next drain, like the old SIGWINCH
                # latch did. #828.)
                ...
                if globals().get('g_resize_flag', False):
                    ...
                    # full redraw above recomputes ``_layout_for``, whose
                    # signature now carries the new ``cols``/``rows`` — the
                    # next drain's ``_fire_resize_if_layout_changed`` sees
                    # the change and fires.
                ...
                if globals().get('g_resize_flag', False):
                    ...
                    # forced redraw recomputes it with the new dimensions.
                if globals().get('g_screen_lost_flag', False):
                    globals()['g_screen_lost_flag'] = False
                    self._pane_cache.clear()
                    self._needs_redraw.add('all')
