...
def _preview_rows(browser):
    ...
    Reads the differential renderer's row cache rather than emitted
...
# --- Refresh preserves scroll (passive last-shown-id gate) -----------------


class TestRefreshPreservesScroll(unittest.TestCase):
    """A same-item refresh must not reset the preview scroll.

    ``_do_refresh`` nulls ``_preview_cursor_id`` to force a re-fetch.
    The scroll/help reset is gated on ``_preview_last_shown_id`` (the
    last *shown* non-None id) rather than the cursor-id slot, so a
    refresh — which does not move the cursor — leaves the user's scroll
    untouched. A genuine move to a different item still resets.
    """

    def test_same_item_refresh_keeps_scroll(self):
        b = _make_browser('\n'.join(f'l{i}' for i in range(200)))
        try:
            b._update_preview_for_cursor()          # land on 'a'
            self.assertEqual(b._preview_last_shown_id, 'a')
            b._preview_scroll = 40
            # Simulate ``_do_refresh``: force a re-fetch by nulling the
            # cursor-id slot while the cursor stays on 'a'.
            b._preview_cursor_id = None
            b._update_preview_for_cursor()
            self.assertEqual(b._preview_scroll, 40,
                             'same-item refresh must not reset scroll')
            self.assertEqual(b._preview_cursor_id, 'a',
                             'identity slot re-syncs to the cursor id')
        finally:
            b.stop_workers()

    def test_refresh_in_help_mode_keeps_help(self):
        b = _make_browser('a\nb\n')
        try:
            b._update_preview_for_cursor()
            b._help_mode = True
            b._preview_cursor_id = None              # refresh forces re-fetch
            b._update_preview_for_cursor()
            self.assertTrue(b._help_mode,
                            'a refresh must not dismiss the help overlay')
        finally:
            b.stop_workers()

    def test_genuine_move_still_resets_scroll(self):
        b = _make_browser('a\nb\n')
        try:
            ix = _data.to_item(Item(id='x'))
            iy = _data.to_item(Item(id='y'))
            ...
            iy.preview = 'yy\n' * 10
            b._state.cursor = 0
            b._update_preview_for_cursor()           # show x
            b._preview_scroll = 15
            b._state.cursor = 1
            b._update_preview_for_cursor()           # move to y
            self.assertEqual(b._preview_scroll, 0,
                             'moving to a different item resets scroll')
        finally:
            b.stop_workers()
    ...
    def test_full_refresh_bounce_restores_scroll(self):
        ...
        # scope row and back. The cursor's sticky anchor keeps pointing at
        # the deep row throughout, so the restore slot survives the bounce
        # and re-applies on return (the "voice umbrella resets on Ctrl-R"
        # regression).
        ...
        try:
            ...
            b._preview_scroll = 6
            b._cursor_anchor = ['V']               # cursor sticks to V
            self.assertEqual(b._preview_last_shown_id, 'V')

            # Simulate ``_do_refresh``: capture the slot, null the cursor
            # id. The anchor keeps V as its primary across the reload —
            # that's what brings the cursor back.
            ...
            self.assertIsNotNone(b._preview_scroll_restore,
                                 'slot kept: the anchor still targets V')
    ...
    def test_user_retarget_voids_restore(self):
        # When the user re-targets the cursor (the sticky anchor primary
        # changes away from the refresh row), the restore slot is dropped
        # so the new row opens fresh and a later revisit does not reuse the
        # remembered scroll.
        b = _make_browser('body')
        try:
            for id_ in ('S', 'V', 'W'):
            ...
            b._preview_cursor_id = None
            # User navigated to W: the anchor now sticks to W.
            b._cursor_anchor = ['W']
            b._state.cursor = 2
            b._update_preview_for_cursor()
            self.assertIsNone(b._preview_scroll_restore,
                              'a sticky-id re-target voids the restore slot')
            self.assertEqual(b._preview_scroll, 0, 'fresh view on the new row')
        finally:
            b.stop_workers()

    def test_pin_voids_restore(self):
        # A home/end pin makes the anchor primary a sentinel — a re-target
        # away from the concrete refresh row, so the slot is dropped.
        b = _make_browser('body')
        try:
            for id_ in ('S', 'V'):
                it = _data.to_item(Item(id=id_))
                ...
                b._state._items_by_id[id_] = it
            b._state._children[None] = [
                b._state._items_by_id[i] for i in ('S', 'V')]
            _state.mark_visible_dirty(b._state)
            b._state.cursor = 1
            b._update_preview_for_cursor()
            b._preview_scroll = 6
            ...
            b._preview_scroll_restore = ('V', 6)   # refresh started on V
            b._preview_cursor_id = None
            b._cursor_anchor = [_state.PIN_FIRST]  # user pressed Home
            b._state.cursor = 0
            b._update_preview_for_cursor()
            self.assertIsNone(b._preview_scroll_restore,
                              'a pin anchor voids the restore slot')
        finally:
            b.stop_workers()
...
class TestStreamingClampPreservesScroll(unittest.TestCase):
    ...
    def _scrolled_browser(self):
        b = _make_browser('\n'.join(f'l{i}' for i in range(200)))
        b._update_preview_for_cursor()          # land on 'a'
        b._preview_visit_delivered = True        # content delivered
        b._preview_scroll = 40
    ...
    def test_tail_pin_rides_growth_not_offset(self):
        ...
        try:
            ...
            _render_preview(b)
            rows = '\n'.join(_preview_rows(b))
            self.assertIn('x2', rows, 'tail-follow rides the new tail')
            self.assertNotIn('l40', rows)
        finally:
            b.stop_workers()
